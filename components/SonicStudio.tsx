            ...
            <div className="w-64 h-full border-r border-white/5 bg-[#1c1c1e]/80 backdrop-blur-3xl flex flex-col z-10">
                ...
                <div className="flex flex-col gap-1 p-4">
                    <button 
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-semibold transition-all active:scale-[0.98] ${activeTab === 'factory' ? 'bg-white text-black shadow-lg' : 'text-white/50 hover:bg-white/10 hover:text-white'}`}
                    >
                        <Mic2 size={16} /> 声音工厂
                    </button>
                    <button 
                        onClick={() => setActiveTab('transcribe')}
                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-semibold transition-all active:scale-[0.98] ${activeTab === 'transcribe' ? 'bg-white text-black shadow-lg' : 'text-white/50 hover:bg-white/10 hover:text-white'}`}
                    >
                        <FileText size={16} /> 语音转文字
                    </button>
                    <button 
                        onClick={() => setActiveTab('live')}
                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-semibold transition-all active:scale-[0.98] ${activeTab === 'live' ? 'bg-white text-black shadow-lg' : 'text-white/50 hover:bg-white/10 hover:text-white'}`}
                    >
                        <MessageCircle size={16} /> 实时对话
                    </button>
                </div>
                    ...
                    <div className="max-w-5xl mx-auto flex flex-col gap-8">
                                        ...
                                        <div className="relative group flex-1">
                                            ...
                                            <div className="relative bg-[#1c1c1e] border border-white/10 rounded-2xl p-5 shadow-2xl flex flex-col h-full min-h-[240px]">
                                                ...
                                                <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/5">
                                                    ...
                                                    <button onClick={handleGenerateClick} disabled={!textPrompt.trim() || isGenerating} className={`px-6 py-3 rounded-2xl font-semibold text-sm shadow-lg transition-all flex items-center gap-2 active:scale-[0.98] ${!textPrompt.trim() || isGenerating ? 'bg-white/10 text-white/30 cursor-not-allowed' : 'bg-white text-black hover:bg-white/90 shadow-white/10'}`}>
                            ...
                            <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                                ...
                                <div className="flex justify-center">
                                    <button 
                                        ...
                                        className={`px-8 py-3.5 rounded-2xl font-semibold text-sm shadow-lg transition-all flex items-center gap-2 active:scale-[0.98] ${!transcribeFile || isTranscribing ? 'bg-white/10 text-white/30 cursor-not-allowed' : 'bg-white text-black hover:bg-white/90 shadow-white/10'}`}
                                    >
                        ...
                        {activeTab === 'live' && (
                            <div className="flex flex-col items-center justify-center gap-10 py-10 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                                ...
                                <button 
                                    onClick={toggleLive}
                                    className={`px-10 py-4 rounded-2xl font-semibold text-base shadow-xl transition-all flex items-center gap-3 active:scale-[0.98] ${isLiveActive ? 'bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500 hover:text-white' : 'bg-white text-black hover:bg-white/90 shadow-white/10'}`}
                                >
                    ...
                    <div className="h-24 bg-[#1c1c1e]/90 backdrop-blur-3xl border-t border-white/10 flex items-center px-8 gap-8 relative z-20">
                        ...
                        <div className="flex-1 flex flex-col items-center gap-2 max-w-2xl mx-auto">
                            <div className="flex items-center gap-6">
                                <button onClick={togglePlay} disabled={!currentTrack} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all active:scale-95 ${currentTrack ? 'bg-white text-black hover:bg-white/90 shadow-lg shadow-white/10' : 'bg-white/10 text-white/30'}`}>
