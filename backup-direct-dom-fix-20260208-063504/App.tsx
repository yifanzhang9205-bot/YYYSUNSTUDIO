          ...
          {contextMenu.visible && (
              <div className="fixed z-[100] bg-white/85 backdrop-blur-xl border border-gray-200/70 rounded-xl shadow-2xl p-1.5 min-w-[140px] animate-in fade-in zoom-in-95 duration-200 origin-top-left" style={{ top: contextMenu.y, left: contextMenu.x }} onMouseDown={(e) => e.stopPropagation()}>
                  ...
                  {contextMenuTarget?.type === 'group' && (
                      <>
                           <button className="w-full text-left px-2.5 py-1.5 text-[10px] font-bold text-gray-600/80 hover:bg-gray-100/80 rounded-lg flex items-center gap-1.5 transition-colors mb-0.5" onClick={() => { saveGroupAsWorkflow(contextMenu.id); closeContextMenu(); }}> <FolderHeart size={11} className="text-gray-500/70" /> 保存为工作流 </button>
